...
\bsref{sec:basics_parallelism}. \bsref{sec:basics_concurrency_parallelism_def} presents the definitions of concurrency and parallelism definitions employed within this project.
...
Based on the considerations presented here we give the definition of concurrency shown in \bsref{def:concurrency}.

\begin{defn}\label{def:concurrency}
\emph{Concurrency is executing multiple task simultaneously}
\end{defn}
as well as the definition of concurrency presented in \bsref{def:parallelism}.\andreas{Move this up, so the defs are together?}
