...
This chapter contains an analysis of prior work related to concurrency models. It will be conducted by investigating papers, articles, and other research material of relevance, in order to discover state of the art, and open problems within the area of concurrency models. The purpose of the analysis is to establish an overview which will be used to choose the further path of our investigation.
\label{chap:preliminary_analysis}
%
\section{Related work}
\lone[inline]{Hvad er current state of the art, related work, open problem?}
The report ``A Study in Concurrency''\cite{damborg2006concurrency} investigates the concurrency support in current object-oriented languages. In specific, four languages were analyzed: Java, C$\omega$, Fortress, and E. They conclude, that there is a mismatch between the object-oriented paradigm and concurrency, and no simple solution for this \cite[p. 101]{damborg2006concurrency}. They claim, that in the current concurrency models, the programmer is, in large, left alone when concurrency is needed. With these discoveries taken into account, a focus on concurrency models which does not require special focus from the programmer could be of special interest for our investigation.
\andreas{Find ny kilde, der siger at programmøren er overladt til sig selv, når det kommer til concurrent programmering}
The landscape of Parallel Computing Research\\



\section{Models to investigate}
\lone[inline]{Hvordan har vi valgt de modeller vi vil undersøge? Fordi det er current trends}
\andreas[inline]{Redegør for relevansen for valg af de enkelte modeller}
