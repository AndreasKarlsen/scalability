...
As described in \bsref{subsec:tl_lock_hard} and \bsref{subsec:tl_composability} the \ac{TL} concurrency model has a number of issue which contributes negatively to its simplicity. When taking multiple locks, taking them in incorrect order might result in a deadlock. Locking is however, as describe in \bsref{subsec:tl_lock_hard}, a global property and the correct order might not be directly discernible by the reader, from within the local context. As such it is no easy task for the reader to reason about whether the order of locking is correct.

\bsref{subsec:tl_composability} describes how how the \ac{TL} model is not composable. Implementing the observer pattern in a thread safe manner was shown to entail a number of issues.
...
\subsubsection{Simplicity}
 The programmer is largely left alone to ensure that a concurrent implementation executes correctly and there for also left alone to tackle the issues that the \ac{TL} model has, limiting its simplicity.
...
Based on these observations we say that the \ac{TL} concurrency model has low writability. The placement of the \ac{TL} concurrency model on the low - high writability spectrum is depicted in \bsref{fig:char_tl_writability}.
