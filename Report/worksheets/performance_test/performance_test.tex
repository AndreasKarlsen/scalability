...
In order to ensure a fair comparison between the implementations execution time, the amount of work they do must be equivalent. As they implementations share the same code base for cluster calculations, it is only the concurrency mechanism and driving skeleton that we must ensure does the same. To verify this, we have made a static set of vectors, which we can run across on all the implementations, and compare the result of the final means. We can verify, that the result is the same, and while we cannot guarantee that the work is identical, as there is multiple ways to get to the same result, this is a positive indication that the same work is performed across the implementations. 
