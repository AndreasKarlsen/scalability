...
\kasper[inline]{Distance measure}


\subsection{Synchronization Intensive Test}
...
Test $C$ is based on a fixed size data set, while the number of mapper tasks will be scaled up.  The test will be based on a data set of 10,000 vectors. Five clusters are created, using 1 to 10 mapper tasks. The algorithm will run for 100,000 iterations. The amount of mappers, have been chosen with consideration to the amount of physical and logical cores in hardware used for the test, described further in \bsref{subsec:hardware}. Our implementation uses one thread to coordinate and drive the calculations. Consequently, making 3 mapper tasks will utilise 4 cores concurrently.

Test $C$ is be based on a fixed number of concurrent tasks as well as a fixed size dataset, while the number of iterations will be scaled up.  Synchronization is applied whenever mappers hand over their work to the reducer. Scaling up the number of iterations will scale up the number of times synchronization code for each of the selected models is executed. As such this test seeks to investigate the impact each of the selected concurrency model has on the performance of the clustering algorithm. It is our expectation that each model will have fairly close but different performance overheads. The test will employ a dataset of 10,000 vectors along with seven mapper tasks. The number of iterations will be scaled from 10,000 to 100,000 in intervals of 10,000. The amount of mappers was chosen to be seven, since it will enable all 8 logical cores to work concurrently.
