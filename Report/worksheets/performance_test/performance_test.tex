...
This section will present and analyse the result of the performance tests. The tests were conducted as described in \bsref{sec:test_approach}. The purpose of the analysis is verify the expectations we had, and establish knowledge that can be used to compare the concurrency models with each other.
\andreas[inline]{Talk about properties of the different models that could affect the result? Fine grained control of locking. STM is automatic resolving, actor is copy by value}

\subsection{Work Intensive Tests}
\kasper[inline]{Akka passes by reference = faster, especially with fast data}
The goal of test $A$ was to determine how the concurrency models is affected with an increased amount of concurrent work. This was done by having 2,000,000 vectors as data, scaling 1 - 10 mappers, running 100 iterations, and a computational demanding distance measure. The results in table \bsref{table:test_results_a} show little to no difference between the models when the scaling the mappers. This is visualised in \bsref{fig:value_norm_testa}. As expected, an increased difference is seen as the amount of mappers are scaled. From 8 to 10 mappers there is some difference between the models, although it is small, the actor model is slower than the other models. This difference is visualised in \bsref{fig:test_results_a_iterations}. Conclusively, the models scale almost identically to the amount of concurrency. The average shown in \bsref{fig:test_results_a_iterations} also confirm this.
%
\begin{center}
\begin{table}[h]
\centering
...
\caption{Test $A$. Visual representation of results for scaling number of mappers}\label{fig:test_results_a_iterations}
\end{figure}
%
\begin{figure}[h]
\centering
...
%
The goal of test $B$ was to determine how the concurrency models scaled with added workload. This was done by scaling the data amount with 1,000,000 to 5,000,000 vectors, having 7 mappers, 100 iterations, and a computational demanding distance measure. The results in table \bsref{table:test_results_b} shows the performance of all three concurrency models is almost equal when scaling the vectors. This is contrary to our initial expectation, as we thought the difference would decrease as the vector size increased.\andreas{Any ideas why?} \bsref{fig:value_norm_testb} visualise the differences of the value-normalized execution time.\andreas{Unexplainable increase in difference in 6 and 7 vectors} The average confirms that overall there is little to no difference between the concurrency models when scaling the vectors.
%
\FloatBarrier
\begin{center}
\begin{table}[h]
...
\end{figure}
%
Overall we can conclude, that the performances of the concurrency models under high workload does not differ much. Neither when scaling the workload or concurrency. The difference is neglectable, unless performance is vital. If that is the case, \ac{TL} and \ac{STM} is marginally preferable in highly concurrent tasks, and the actor model in tasks with large data amounts.
%
\FloatBarrier
\subsection{Synchronization Intensive Tests}
%TEST C
...
%
The goal of test $C$ was to see the impact of scaling the concurrent work in a synchronization intensive task. This was done by scaling 1 - 10 mappers, a data size of 10,000 vectors, 100,000 iterations, and a computational simple distance measure. The results displayed in \bsref{table:test_results_concurrent_tasksc} show that when the amount of synchronization increases, the differences between the concurrency models show. \bsref{fig:test_results_concurrent_tasks_c} visualises this tendency, and shows that the overall execution time follows the same development. As expected, it shows a huge decrease between 1 and 3 mappers, and a continued decrease until 7 mappers. \ac{TL} is fastest in all the cases, and in the fastest case with 7 mappers, \ac{STM} and the actor model is 4\% and 7\% slower respectively. In the task with most synchronization with 10 mappers, \ac{STM} and the actor model was 12\% and 10\% slower respectively. The difference in performance between the models is increasing as the amount of synchronization is scaled up. \bsref{fig:value_norm_testc} visualises this, and shows that in the average case, \ac{TL} is the fastest, and \ac{STM} and the actor model is 6\% and 5\% slower relatively.
\andreas{Put these goals up under the test definition}
%
\begin{center}
\begin{table}[h]
\centering
...
\end{figure}
%
Overall we can conclude, that the performances of the concurrency models under synchronization intensive workload does vary. When scaling the concurrency, the \ac{TL} is performing best, with a margin to actor and \ac{STM} which is close. However, even though \ac{TL} is the fastest, the difference in the worst case is only 12\%, and in average 5\% to 6\%. When scaling the iterations, \ac{TL} was in average marginally faster, althought it is neglectable unless performance is vital.
%
