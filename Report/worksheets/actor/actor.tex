...
%the actor model assums message will be delivered eventaully, but that is the conceptual model and in a real implementation faliures can cause starvation and then it is up to the indivivudal implementation of the actor model to adhere the principles of the actor model. (en af kilderne) actors in scala kap. 2.5 (anden sidste paragraf)
...
%actor lifecycle created and then it starts, start = awaiting message, stop = end  %måske kig på figur 12 i 8.2 JVM Mastering
...
%Forklar modellen
	%While this is happening there may be other actors concurrently running sending messages around and processing.
