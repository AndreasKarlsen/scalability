...
These high level abstractions enable the programmer to model and think about concurrent problems conceptually in terms of actors and their interaction, without having to write the program.

Based on the above considerations we say that the actor model has a high level of abstraction, as illustrated on \bsref{fig:actor_level_of_abstraction}.
...
However, the conceptual actor model does not provide constructs to prune non-determinism which means that the programmer must build patterns for such coordination manually. This affects the expressivity of the actor model negatively, but most actor implementations typically have some useful coordination patterns, such as the request-reply pattern, which reduces the impact on expressivity.
