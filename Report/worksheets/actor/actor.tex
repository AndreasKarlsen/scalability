...
The expressivity relates closely to the level of abstraction. In the actor model you do not have to explicitly mark which state variables should be ensured mutual exclusion, instead it is done for all state variables implicitly as a result of the design of the model. Fur

%furtore more also does not have to start the underlying threads. The programmer only have to deal with the abstraction of actors. 
	
%also very expressive in regard to that you only need the name of another actor, and then the physicial location is up to the actor implementation to figure out which also enables easy distribution of actors onto a cluster of machines (in Akka it is just a matter of specifing it in the config file)
	%enables the developer to avoid much of the difficulty in regards to distributed computing (implementation specific)
	%makes the developer able to program the system on his own computer and then just by chaning a simple config file of where it should run, can run distributed.
	
\missingfigure{Høj expressivitiy - ved ikke hvor høj}
