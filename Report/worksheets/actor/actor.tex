...
Actors are independent concurrently running entities that communicate with each other solely through asynchronous message passing\cite[p. 304]{tasharofi2013scala}. Upon receiving a message the actor examines it and determines how it will react, which can be in a number of ways\cite[p. 2]{hewitt2014actor}:\kasper{Er det kun de tre her? I så fald burde det måske være "in one of the following ways" i stedet for "a number of ways".}
\begin{enumerate}
...
\subsection{Title something - look in comments}
%Hard to model with threads but just because we use actors we do no remove this problem entirely - we still have to model it with actors
%evt. kig på Bent's præsentation

\subsection{Adhering to Actor Properties}
\toby[inline]{Måske hav den i implementations afsnittet og tag udgangspunkt i jvm comparsion of actors artiklen til at beskrive problematikken. Evt. prøv at finde ud af hvilke semantic properties akka understøtter}
...
%Også i forhold til mapper en actor til en jvm thread eller light weight actors
