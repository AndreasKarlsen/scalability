...
As discussed in \bsref{ssec:actor_modelling}, there is an overhead by creating, sending and receiving messages, as a result of asynchronous message passing as the only communication form. So there should not be too many small messages, but also not few large messages as that will hurt the dynamics among actors. We do not find this as severe to the simplicity of the actor model as the other issues discussed, because no matter what concurrency model you use, you must be careful not to assign too much work to a single entity. \toby{Hvad siger i til det her afsnit?}
%However, message size and load can be crucial to obtaining an efficient program.

The placement of the actor model on the simplicity spectrum is shown on BLA\toby{indsæt ref}. The actor model provides a fairly simple concurrency model with implicit concurrency that avoids race conditions by design and helps avoid deadlocks. However, there are also a number of issues discussed above which has a negative impact on the simplicity. We have therefore placed the actor model in the middle of the spectrum.\toby{Bliv enige om hvor den skal stå. Jeg er selv ikke sikker. Evt. begrund det med explicit concurrency og den abstraction man får - men stadivæk ikke nemt at lave concurrency}
...
%Forhold sig til hver punkt for writability
	%simplicity
	%orthogonality
	%level of abstraction
	%expressivity
	
%Forhold sig til concurrency issues også - evt. generelt som TL har gjort


%the implicit mututal exclusion and race condition free of all state variables makes it easiyer to write, as the programmer does not have to worry about which state varaibles should be covered and which should not
	%ensure race condition free by design is nice
	%but also does not allow for some state varaibles to not be covered by it, which may be usefull for values that are not changed, so they can be accessed by more entities than one at the same time (which is not possible in the actor model)
...
%især i nogle situationer fx. stock broker, hvor man gerne prune non-determinism og især påvirker writability negativ i implementationer der ikke har pattern for dette.
	%mere kompliceret coordination hvis man holder sig kun til aync message passing
	%mange situationer kan man ommudulere det så at man ikke behøves block behaviour (quoten fra scala in actors)
	
%orthogonality
	%few constructs, so easy overview them and they can be combined in a large number of ways which affect the writability positively (however somethings are not possible - such as having some state variables that allows mutiple entities to access at the time)
	
%high level of abstraction and expressivity influcenes the writability positively.
	%programmer does not have to worry about the complications involved with threads and physical location of actors or mailboxes.
