...
One of the main differences from other concurrency models, such as \ac{TL} and \ac{STM}, is that there is no notion of shared state in the actor model. Instead each actor has isolated state where only the actor itself is responsible and allowed to modify state. An actor can only affect the state of another actor indirectly by message passing and the receiving actor is then responsible to change the state. The reason that message passing as the only communication form and isolated state is preferable over shared state is that race conditions are avoided by design.\toby{Skal der være en reference tilbage hvor vi snakker om det før?} To exemplify this imagine two messages that wants to read and modify a state variable of an actor. The actor processes each message in one isolated atomic operation which means that the messages will not interfere with one another as the message will be processed one at a time.
%subsec:race_coditions

The issue of deadlocks\toby{Eller er det livelocks i forhold til vores tidligere definitioner? Skal nok lige have kasper på banen her} are however also present in the actor model. In the shared memory concurrency models a deadlock is often raised as a result of multiple threads trying to acquire resources 

The actor model alleviates the race condition problem but deadlocks is still possible 

%deadlock eller livelock

%Starvation og priority inversion
...
%race condition free
% If processes communicate only by passing messages, and those messages are immutable, then data races are avoided by design.
...
%ingen race conditions by nature of model
%still possible to have deadlocks (eller er det livelocks?)
...
\subsection{Not a silver bullet - look in comments}
...
\subsection{Actor thread mapping}
%Måske have et afsnit i discussion i actor om actor mapping til single thread vs lightweight actors der mapper mange actors på en tråd.
