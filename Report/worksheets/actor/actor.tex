...
Actors can be modelled and combined in a way that produces deadlocks, as discussed in \bsref{ssec:actor_deadlock}. Such combinations are not desirable and if possible such combinations should be detected and avoided. This hurts the orthogonality of the actor model as some of the combinations are not feasible. However, recall that actors in the system not part of the deadlock are not affected and can continue to run, which means violating the allowed combinations is not as fatal as in other concurrency models that stop the progress of the rest of the system.\toby{ved ikke om jeg skal ahve den sidste sætning med}
